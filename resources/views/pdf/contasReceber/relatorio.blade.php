...
<style>
    ...
    th, td { padding: 6px; text-align: center; font-size: 7pt; }
    th { background: #f2f2f2; }
    h1 { text-align: center; font-size: 14pt; margin-bottom: 6px; }
    .text-right { text-align: right; }
    .text-center { text-align: center; }
    .filtros { margin-bottom: 18px; font-size: 1rem; color: #888; background: #f4f6f9; padding: 8px 12px; border-radius: 6px; }
    .filtros strong { display: block; margin-bottom: 4px; }
    .filtros span { margin-right: 12px; }
    .resumo { margin-top: 20px; display: flex; gap: 20px; }
    .resumo table { margin-top: 0; }
    ...
    <div class="filtros">
        ...
        @foreach ($filtrosNomes as $chave => $valor)
            @if ($valor)
                ...
                @endif
                <span><b>{{ $chave }}:</b>
                    @if (str_contains($chave, 'Data') || str_contains($chave, 'Vencimento') || str_contains($chave, 'Recebimento'))
...
<table>
    ...
    <tbody>
        ...
        @foreach($contas as $c)
            @php
                ...
                $totalRecebido += (float) $c->valor_recebido;
                if($c->status) {
                    $qtdRecebidas++;
                } else {
                    $qtdPendentes++;
            ...
            <tr>
                ...
                <td class="text-right">R$ {{ number_format((float) $c->valor_recebido, 2, ',', '.') }}</td>
    ...
    <tfoot>
        <tr style="font-weight:bold; background:#f2f2f2;">
            ...
            <td class="text-right">R$ {{ number_format($totalRecebido, 2, ',', '.') }}</td>
            <td colspan="3"></td>
        </tr>
    </tfoot>
</table>
...
<div style="margin-top: 20px; font-weight: bold;">
    ...
    Recebidas: {{ $qtdRecebidas }} | 
    Pendentes: {{ $qtdPendentes }}
</div>

@php
    $porCliente = $contas->groupBy(fn($it)=> $it->cliente->nome ?? '---')->map->sum('valor_parcela')->sortDesc();
...
<div class="resumo">
    <div style="flex:1;">
        ...
        <table>
            ...
            <tbody>
                @foreach($porCliente as $nome => $valor)
                    <tr>
                        <td>{{ $nome }}</td>
